...
import { getProducts, removeProduct, toggleDelete } from "../feature/products/productsSlice";


const ProductList = () => {
  const products= useSelector((state) => state.products.products.data);
  const {isLoading, isError, deleteSuccess, error}= useSelector((state) => state.products);
...
useEffect(()=>{
   ...
   if(!isLoading && deleteSuccess){
    toast.success("deleted success", {id:"deleteProduct"})
   }
   if(!isLoading && isError){
    toast.error("deleted error", {id:"deleteProduct"})
   }
},[isError, isLoading, error, deleteSuccess])
